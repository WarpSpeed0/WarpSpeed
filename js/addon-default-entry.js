...
/***/ "./node_modules/url-loader/dist/cjs.js!./src/addons/addons/onion-skinning/play.svg":
/*!*****************************************************************************************!*\
  !*** ./node_modules/url-loader/dist/cjs.js!./src/addons/addons/onion-skinning/play.svg ***!
  \*****************************************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony default export */ __webpack_exports__["default"] = ("data:image/svg+xml;base64,");
...
/* harmony import */ var _url_loader_play_svg__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! url-loader!./play.svg */ "./node_modules/url-loader/dist/cjs.js!./src/addons/addons/onion-skinning/play.svg");
/* generated by pull.js */
...
const resources = {
  ...
  "play.svg": _url_loader_play_svg__WEBPACK_IMPORTED_MODULE_6__["default"]
};
...
/* harmony default export */ __webpack_exports__["default"] = (async function (_ref) {
  ...
  const USER_LAYER_CHANGE_EVENT = "scratch-paint/tw-user-layers-change";
  const LAYERED_SVG_REGEX = /<!--twLayeredPaint:([A-Za-z0-9+/=]+)-->/;
  const SVG_FONT_FAMILY_REGEX = /font-family(?!="none")/;
  let originalProjectImportJSON = null;
  ...
  const normalizeLayerSchema = layerSchema => {
    if (!Array.isArray(layerSchema)) {
      return null;
    }
    const usedIds = new Set();
    const normalized = [];
    for (const layer of layerSchema) {
      if (!layer || typeof layer.id !== "string" || !layer.id.trim() || usedIds.has(layer.id)) {
        continue;
      }
      ...
      normalized.push({
        ...
        id: layer.id,
        locked: !!layer.locked,
        name: typeof layer.name === "string" && layer.name.trim() ? layer.name.trim() : "Layer ".concat(normalized.length + 1),
        opacity: Math.max(0, Math.min(1, Number.isNaN(Number(layer.opacity)) ? 1 : Number(layer.opacity))),
        type: layer.type === "bitmap" ? "bitmap" : "vector"
      });
    }
    return normalized.length ? normalized : null;
  };
  const normalizeLayeredProject = layeredProject => {
    if (!layeredProject || !layeredProject.project) {
      return null;
    }
    const normalized = {
      activeLayerId: typeof layeredProject.activeLayerId === "string" ? layeredProject.activeLayerId : null,
      project: cloneJSON(layeredProject.project),
      version: typeof layeredProject.version === "number" ? layeredProject.version : 1
    };
    const normalizedLayers = normalizeLayerSchema(layeredProject.layers);
    if (normalizedLayers) {
      normalized.layers = normalizedLayers;
    }
    return normalized;
  };
  const parseLayeredSvgMetadata = svgString => {
    const match = svgString && svgString.match(LAYERED_SVG_REGEX);
    if (!match) {
      return null;
    }
    try {
      return normalizeLayeredProject(JSON.parse(decodeBase64(match[1])));
    } catch (error) {
      return null;
    }
  };
  const getCostumeLayeredProject = costume => normalizeLayeredProject(costume && costume.twLayeredProject);
  ...
  const createOnionLayer = optTargetLayerId => {
    const layer = new paper.Layer();
    layer.locked = true;
    layer.guide = true;
    layer.data.sa_isOnionLayer = true;
    if (optTargetLayerId) {
      layer.data.sa_targetLayerId = optTargetLayerId;
    }
    return layer;
  };
  ...
  const relayerOnionLayers = () => {
    ...
    const onionLayers = project.layers.filter(i => i.data && i.data.sa_isOnionLayer);
    if (!onionLayers.length) {
      return;
    }
    const userLayers = project.layers.filter(i => i.data && i.data.isUserLayer);
    const userLayerIndexById = new Map(userLayers.map((layer, index) => [layer.data.layerId, index]));
    const backgroundGuideLayer = project.layers.find(i => i.data && i.data.isBackgroundGuideLayer);
    const sortedOnionLayers = onionLayers.slice().sort((a, b) => {
      const aIndex = userLayerIndexById.has(a.data.sa_targetLayerId) ? userLayerIndexById.get(a.data.sa_targetLayerId) : Infinity;
      const bIndex = userLayerIndexById.has(b.data.sa_targetLayerId) ? userLayerIndexById.get(b.data.sa_targetLayerId) : Infinity;
      return aIndex - bIndex;
    });
    for (const onionLayer of sortedOnionLayers) {
      const targetLayerId = onionLayer.data && onionLayer.data.sa_targetLayerId;
      const targetUserLayer = typeof targetLayerId === "string" && userLayerIndexById.has(targetLayerId) ? userLayers[userLayerIndexById.get(targetLayerId)] : null;
      onionLayer.visible = !targetUserLayer || targetUserLayer.visible;
      if (targetUserLayer) {
        if (settings.layering === "front") {
          onionLayer.insertAbove(targetUserLayer);
        } else {
          onionLayer.insertBelow(targetUserLayer);
        }
        continue;
      }
      if (settings.layering === "front") {
        const topUserLayer = userLayers[userLayers.length - 1];
        if (topUserLayer) {
          onionLayer.insertAbove(topUserLayer);
        } else {
          project.addLayer(onionLayer);
        }
      } else {
        const bottomUserLayer = userLayers[0];
        if (bottomUserLayer) {
          onionLayer.insertBelow(bottomUserLayer);
        } else if (backgroundGuideLayer) {
          onionLayer.insertAbove(backgroundGuideLayer);
        } else {
  ...
  const getCurrentSpriteLayerSchema = () => {
    const vm = addon.tab.traps.vm;
    return normalizeLayerSchema(vm && vm.editingTarget && vm.editingTarget.sprite && vm.editingTarget.sprite.twLayerSchema) || [];
  };
  const createTemporaryScope = () => {
    const scope = new paper.PaperScope();
    const paperCenter = getPaperCenter();
    const canvas = document.createElement("canvas");
    canvas.width = Math.max(1, Math.round(paperCenter.x * 2));
    canvas.height = Math.max(1, Math.round(paperCenter.y * 2));
    scope.setup(canvas);
    return scope;
  };
  const cloneCanvas = (sourceCanvas, optSourceRect) => {
    const sourceRect = optSourceRect || {
    ...
    context.drawImage(sourceCanvas, sourceRect.x, sourceRect.y, sourceRect.width, sourceRect.height, 0, 0, sourceRect.width, sourceRect.height);
    return canvas;
  };
  const getCanvasHitBounds = canvas => {
    const {
      width,
      height
    } = canvas;
  ...
  const createOnionRasterFromCanvas = (canvas, position, opacity, isBefore, smoothing) => {
    const raster = new paper.Raster(cloneCanvas(canvas));
    raster.opacity = opacity;
    raster.guide = true;
    raster.locked = true;
    raster.smoothing = smoothing;
    raster.position = new paper.Point(position.x, position.y);
    raster.remove();
  ...
  const makeLayeredOnions = (opacity, layeredProject, isBefore) => {
    const scope = createTemporaryScope();
    (originalProjectImportJSON || scope.project.importJSON).call(scope.project, layeredProject.project);
    scope.project.deselectAll();
    const layerSchema = getCurrentSpriteLayerSchema();
    const effectiveLayerSchema = layerSchema.length ? layerSchema : Array.isArray(layeredProject.layers) ? layeredProject.layers : [];
    const layerInfoById = new Map(effectiveLayerSchema.map(layer => [layer.id, layer]));
    const onionEntries = [];
    for (const layer of scope.project.layers) {
      if (!layer.data || !layer.data.isUserLayer) {
        continue;
      }
      const layerInfo = layerInfoById.get(layer.data.layerId);
      const layerOpacity = layerInfo && typeof layerInfo.opacity !== "undefined" ? Number(layerInfo.opacity) : 1;
      if (!layer.visible || layerInfo && layerInfo.hidden || layerOpacity <= 0) {
        continue;
      }
      if (layerInfo && layerInfo.type === "bitmap" || layer.data.layerType === "bitmap") {
        const sourceRaster = layer.children.find(child => child.data && child.data.isLayerRaster);
        if (!sourceRaster || !sourceRaster.canvas) {
          continue;
        }
        const hitBounds = getCanvasHitBounds(sourceRaster.canvas);
        if (!hitBounds) {
          continue;
        }
        const trimmedCanvas = cloneCanvas(sourceRaster.canvas, hitBounds);
        paper.activate();
        const raster = createOnionRasterFromCanvas(trimmedCanvas, {
          x: hitBounds.x + hitBounds.width / 2,
          y: hitBounds.y + hitBounds.height / 2
        }, opacity * layerOpacity, isBefore, false);
        onionEntries.push({
          item: raster,
          targetLayerId: layer.data.layerId
        });
        scope.activate();
        continue;
      }
      const renderableChildren = layer.children.filter(child => !(child.guide || child.data && child.data.isLayerRaster));
      if (!renderableChildren.length) {
        continue;
      }
      const rasterizedLayer = layer.rasterize(72, false);
      paper.activate();
      const raster = createOnionRasterFromCanvas(rasterizedLayer.canvas, rasterizedLayer.position, opacity * layerOpacity, isBefore, true);
  ...
  const getSelectedCostumeIndex = () => {
    const vm = addon.tab.traps.vm;
    if (!vm || !vm.editingTarget || typeof vm.editingTarget.currentCostume !== "number") {
      return -1;
    }
    return vm.editingTarget.currentCostume;
  };
  const buildOnionDescriptors = (selectedCostumeIndex, costumes, previousCount, nextCount) => {
    const layersToCreate = [];
    const addDirection = (count, direction) => {
      for (let distance = 1; distance <= count; distance++) {
        const index = selectedCostumeIndex + distance * direction;
        if (index < 0 || index >= costumes.length) {
          continue;
        }
        const opacity = (settings.opacity - settings.opacityStep * (distance - 1)) / 100;
        if (opacity <= 0) {
          continue;
        }
        layersToCreate.push({
          index,
          isBefore: direction < 0,
          opacity
        });
      }
    };
    addDirection(previousCount, -1);
    addDirection(nextCount, 1);
  ...
  const updateOnionLayers = async () => {
    ...
    try {
      let layersToCreate = buildOnionDescriptors(selectedCostumeIndex, costumes, settings.previous, settings.next);

      // If the configured direction has no available costumes, fall back to the
      // opposite side so the toggle still shows the nearest costume instead of
      // looking broken on the first or last costume.
      if (!layersToCreate.length) {
        if (settings.previous > 0 && settings.next === 0) {
          layersToCreate = buildOnionDescriptors(selectedCostumeIndex, costumes, 0, settings.previous);
        } else if (settings.next > 0 && settings.previous === 0) {
          layersToCreate = buildOnionDescriptors(selectedCostumeIndex, costumes, settings.next, 0);
        }
      }
      const onions = await Promise.all(layersToCreate.map(_ref3 => {
        ...
        const layeredProject = getCostumeLayeredProject(onionCostume) || (typeof onionAsset === "string" ? parseLayeredSvgMetadata(onionAsset) : null);
        if (layeredProject && layeredProject.project) {
          return makeLayeredOnions(opacity, layeredProject, isBefore);
        }
        if (onionCostume.dataFormat === "svg") {
          return makeVectorOnion(opacity, onionCostume, onionAsset, isBefore).then(item => [{
            item,
            targetLayerId: null
          }]);
        } else if (onionCostume.dataFormat === "png" || onionCostume.dataFormat === "jpg") {
          return makeRasterOnion(opacity, onionCostume, onionAsset, isBefore).then(item => [{
            item,
            targetLayerId: null
          }]);
        } else {
      ...
      if (globalUpdateId === localUpdateId) {
        const onionGroups = new Map();
        for (const entry of onions.flat()) {
          if (!entry || !entry.item) {
            continue;
          }
          const key = entry.targetLayerId || "__global__";
          if (!onionGroups.has(key)) {
            onionGroups.set(key, []);
          }
          onionGroups.get(key).push(entry.item);
        }
        for (const [targetLayerId, items] of onionGroups.entries()) {
          const layer = createOnionLayer(targetLayerId === "__global__" ? null : targetLayerId);
          for (const item of items) {
            layer.addChild(item);
          }
        }
  ...
  let animationPreviewElements = null;
  let animationPreviewData = null;
  let animationPreviewKeydownHandler = null;
  let animationPreviewResizeHandler = null;
  let animationPreviewTimer = null;
  let animationPreviewAudio = null;
  let animationPreviewButton = null;
  let animationPreviewStageAspectLoadNonce = 0;
  let animationPreviewStageAspectRatio = 1;
  ...
  const isAnimationPreviewOpen = () => !!(animationPreviewModal && animationPreviewModal.backdrop && animationPreviewModal.backdrop.isConnected && animationPreviewElements);
  const getAnimationPreviewData = () => {
    const vm = addon.tab.traps.vm;
    ...
    return {
      costumes,
      currentCostumeIndex: clampNumber(Math.max(0, getSelectedCostumeIndex()), 0, costumes.length - 1),
      sounds: Array.isArray(target.sprite.sounds) ? target.sprite.sounds : [],
      spriteName: target.sprite.name || "Sprite"
    };
  };
  const normalizeAnimationPreviewState = previewData => {
    const costumeCount = previewData.costumes.length;
    const defaultStartIndex = clampNumber(previewData.currentCostumeIndex, 0, costumeCount - 1);
    const defaultEndIndex = defaultStartIndex < costumeCount - 1 ? costumeCount - 1 : 0;
    const soundCount = previewData.sounds.length;
    const normalizeCostumeIndex = (value, fallback) => Number.isInteger(value) && value >= 0 && value < costumeCount ? value : fallback;
    animationPreviewState = {
      endCostumeIndex: normalizeCostumeIndex(animationPreviewState.endCostumeIndex, defaultEndIndex),
      fps: clampNumber(Number.isFinite(Number(animationPreviewState.fps)) ? Math.round(Number(animationPreviewState.fps)) : 12, 1, 240),
      soundIndex: Number.isInteger(animationPreviewState.soundIndex) && animationPreviewState.soundIndex >= 0 && animationPreviewState.soundIndex < soundCount ? animationPreviewState.soundIndex : -1,
      startCostumeIndex: normalizeCostumeIndex(animationPreviewState.startCostumeIndex, defaultStartIndex)
    };
    return animationPreviewState;
  };
  const getAnimationPreviewCostumeName = (costume, costumeIndex) => costume && costume.name || "Costume ".concat(costumeIndex + 1);
  const getAnimationPreviewCostumeSize = costume => {
    if (!costume || !Array.isArray(costume.size) || costume.size.length < 2) {
      return null;
    }
    const width = Number(costume.size[0]);
    const height = Number(costume.size[1]);
  ...
  const getAnimationPreviewCostumeUrl = costume => {
    if (!costume || !costume.asset || typeof costume.asset.encodeDataURI !== "function") {
      return "";
    }
    if (costume.dataFormat === "svg" && typeof costume.asset.decodeText === "function") {
      const svgText = costume.asset.decodeText();
      if (SVG_FONT_FAMILY_REGEX.test(svgText)) {
        return "data:image/svg+xml;utf8,".concat(encodeURIComponent(Object(_turbowarp_scratch_svg_renderer__WEBPACK_IMPORTED_MODULE_0__["inlineSvgFonts"])(svgText)));
      }
    }
    return costume.asset.encodeDataURI();
  };
  const getAnimationPreviewSoundUrl = sound => {
    if (!sound || !sound.asset || typeof sound.asset.encodeDataURI !== "function") {
      return "";
    }
    const soundUrl = sound.asset.encodeDataURI();
    const wavPrefix = "data:audio/x-wav;base64,";
    if (!soundUrl.startsWith(wavPrefix)) {
      return soundUrl;
    }
    const mimeByFormat = {
      mp3: "audio/mp3",
    ...
    const mime = mimeByFormat[sound.dataFormat] || "audio/x-wav";
    return "data:".concat(mime, ";base64,").concat(soundUrl.slice(wavPrefix.length));
  };
  const buildAnimationFrameSequence = (startCostumeIndex, endCostumeIndex) => {
    const sequence = [];
    const step = startCostumeIndex <= endCostumeIndex ? 1 : -1;
    for (let index = startCostumeIndex; step > 0 ? index <= endCostumeIndex : index >= endCostumeIndex; index += step) {
      sequence.push(index);
    }
    return sequence;
  };
  const setAnimationPreviewButtonState = () => {
    if (!animationPreviewButton) {
      return;
    }
    animationPreviewButton.dataset.enabled = isAnimationPreviewOpen() ? "true" : "false";
  };
  const setAnimationPreviewStatus = statusText => {
    if (!animationPreviewElements || !animationPreviewElements.status) {
      return;
    }
    animationPreviewElements.status.textContent = statusText;
  };
  const applyAnimationPreviewStageAspectRatio = ratio => {
    if (!animationPreviewElements || !animationPreviewElements.stage) {
      return;
    }
    const stage = animationPreviewElements.stage;
    const previewPanel = stage.parentElement;
    const safeRatio = Number.isFinite(ratio) && ratio > 0 ? ratio : 1;
    const stagePadding = 40;
    const maxStageHeight = window.innerWidth <= 760 ? Math.min(256, window.innerHeight * 0.35) : Math.min(336, window.innerHeight * 0.45);
    const availableWidth = Math.max(140, previewPanel && previewPanel.clientWidth ? previewPanel.clientWidth - 2 : stage.clientWidth || 140);
    const maxContentWidth = Math.max(100, availableWidth - stagePadding);
    let contentWidth = Math.min(maxContentWidth, maxStageHeight * safeRatio);
    let contentHeight = contentWidth / safeRatio;
    if (contentHeight > maxStageHeight) {
      contentHeight = maxStageHeight;
      contentWidth = contentHeight * safeRatio;
    }
    animationPreviewStageAspectRatio = safeRatio;
    stage.style.setProperty("--sa-animation-stage-ratio", String(safeRatio));
    stage.style.width = "".concat(Math.max(140, Math.round(contentWidth + stagePadding)), "px");
    stage.style.height = "".concat(Math.max(140, Math.round(contentHeight + stagePadding)), "px");
  };
  const updateAnimationPreviewStageAspectRatio = (previewData, costumeIndex) => {
    if (!animationPreviewElements || !animationPreviewElements.stage) {
      return;
    }
    const costume = previewData && previewData.costumes ? previewData.costumes[costumeIndex] : null;
    const costumeSize = getAnimationPreviewCostumeSize(costume);
    if (costumeSize) {
      applyAnimationPreviewStageAspectRatio(costumeSize.width / costumeSize.height);
      return;
    }
    const costumeUrl = getAnimationPreviewCostumeUrl(costume);
    if (!costumeUrl) {
      applyAnimationPreviewStageAspectRatio(1);
      return;
    }
    const loadNonce = ++animationPreviewStageAspectLoadNonce;
    const ratioProbe = new Image();
    ratioProbe.onload = () => {
      if (loadNonce !== animationPreviewStageAspectLoadNonce || !isAnimationPreviewOpen()) {
        return;
      }
      const width = ratioProbe.naturalWidth || ratioProbe.width;
      const height = ratioProbe.naturalHeight || ratioProbe.height;
      applyAnimationPreviewStageAspectRatio(width > 0 && height > 0 ? width / height : 1);
    };
    ratioProbe.onerror = () => {
      if (loadNonce === animationPreviewStageAspectLoadNonce) {
        applyAnimationPreviewStageAspectRatio(1);
      }
    };
    ratioProbe.src = costumeUrl;
  };
  const setAnimationPreviewFrame = (previewData, costumeIndex, frameNumber, frameCount) => {
    if (!animationPreviewElements) {
      return;
    }
    const costume = previewData.costumes[costumeIndex];
    if (!costume) {
      return;
    }
    const costumeName = getAnimationPreviewCostumeName(costume, costumeIndex);
    if (animationPreviewElements.image) {
      animationPreviewElements.image.src = getAnimationPreviewCostumeUrl(costume);
      animationPreviewElements.image.alt = costumeName;
    }
    if (animationPreviewElements.frameName) {
      animationPreviewElements.frameName.textContent = "Frame ".concat(frameNumber, " of ").concat(frameCount, ": ").concat(costumeName);
    }
  };
  const stopAnimationPreviewPlayback = function stopAnimationPreviewPlayback() {
    let {
    ...
    if (animationPreviewAudio) {
      animationPreviewAudio.pause();
      animationPreviewAudio.currentTime = 0;
      animationPreviewAudio = null;
    }
    if (!preserveStatus) {
      setAnimationPreviewStatus("Stopped.");
    }
  };
  const updateAnimationPreviewSnapshot = previewData => {
    const frameSequence = buildAnimationFrameSequence(animationPreviewState.startCostumeIndex, animationPreviewState.endCostumeIndex);
    const frameCount = Math.max(1, frameSequence.length);
    const firstFrameIndex = frameSequence.length ? frameSequence[0] : animationPreviewState.startCostumeIndex;
    updateAnimationPreviewStageAspectRatio(previewData, firstFrameIndex);
    setAnimationPreviewFrame(previewData, firstFrameIndex, 1, frameCount);
    setAnimationPreviewStatus("Ready to play ".concat(frameCount, " frame").concat(frameCount === 1 ? "" : "s", " at ").concat(animationPreviewState.fps, " FPS."));
  };
  const readAnimationPreviewStateFromModal = () => {
    if (!animationPreviewElements) {
      return animationPreviewState;
    }
    const readInteger = (element, fallback) => {
      const value = element ? parseInt(element.value, 10) : fallback;
      return Number.isFinite(value) ? value : fallback;
    };
    animationPreviewState = _objectSpread(_objectSpread({}, animationPreviewState), {}, {
      endCostumeIndex: readInteger(animationPreviewElements.endSelect, animationPreviewState.endCostumeIndex),
      fps: clampNumber(readInteger(animationPreviewElements.fpsInput, animationPreviewState.fps), 1, 240),
      soundIndex: readInteger(animationPreviewElements.soundSelect, animationPreviewState.soundIndex),
      startCostumeIndex: readInteger(animationPreviewElements.startSelect, animationPreviewState.startCostumeIndex)
    });
    return animationPreviewState;
  };
  const closeAnimationPreviewModal = () => {
    stopAnimationPreviewPlayback({
      preserveStatus: true
    });
    if (animationPreviewKeydownHandler) {
      window.removeEventListener("keydown", animationPreviewKeydownHandler);
      animationPreviewKeydownHandler = null;
    }
    if (animationPreviewResizeHandler) {
      window.removeEventListener("resize", animationPreviewResizeHandler);
      animationPreviewResizeHandler = null;
    }
    if (animationPreviewModal) {
      animationPreviewModal.remove();
    }
    animationPreviewModal = null;
    animationPreviewElements = null;
    animationPreviewData = null;
    animationPreviewStageAspectLoadNonce += 1;
    animationPreviewStageAspectRatio = 1;
    setAnimationPreviewButtonState();
  };
  const createAnimationPreviewField = function createAnimationPreviewField(labelText, control) {
    let {
      noteText = null,
      wide = false
    } = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    const field = document.createElement("label");
    field.className = "sa-animation-field";
    if (wide) {
      field.dataset.wide = "true";
    }
    const label = document.createElement("span");
    label.className = "sa-animation-field-label";
    label.textContent = labelText;
    field.appendChild(label);
    field.appendChild(control);
    if (noteText) {
      const note = document.createElement("span");
      note.className = "sa-animation-field-note";
  ...
  const playAnimationPreview = () => {
    if (!animationPreviewData || !isAnimationPreviewOpen()) {
      stopAnimationPreviewPlayback({
        preserveStatus: true
      });
      setAnimationPreviewStatus("No costumes available.");
      return;
    }
    readAnimationPreviewStateFromModal();
    normalizeAnimationPreviewState(animationPreviewData);
    const frameSequence = buildAnimationFrameSequence(animationPreviewState.startCostumeIndex, animationPreviewState.endCostumeIndex);
    if (!frameSequence.length) {
      setAnimationPreviewStatus("No frames selected.");
      return;
    }
    stopAnimationPreviewPlayback({
      preserveStatus: true
    ...
    const fps = animationPreviewState.fps;
    let framePointer = 0;
    updateAnimationPreviewStageAspectRatio(animationPreviewData, frameSequence[0]);
    setAnimationPreviewFrame(animationPreviewData, frameSequence[0], 1, totalFrames);
    setAnimationPreviewStatus("Playing ".concat(totalFrames, " frame").concat(totalFrames === 1 ? "" : "s", " at ").concat(fps, " FPS."));
    if (animationPreviewState.soundIndex >= 0 && animationPreviewState.soundIndex < animationPreviewData.sounds.length) {
      const soundUrl = getAnimationPreviewSoundUrl(animationPreviewData.sounds[animationPreviewState.soundIndex]);
      if (soundUrl) {
        animationPreviewAudio = new Audio(soundUrl);
        animationPreviewAudio.play().catch(() => {
          setAnimationPreviewStatus("Playing ".concat(totalFrames, " frame").concat(totalFrames === 1 ? "" : "s", " at ").concat(fps, " FPS. Audio could not start."));
        });
      }
    }
    if (totalFrames === 1) {
    ...
    const advanceFrame = () => {
      ...
      framePointer += 1;
      if (framePointer >= totalFrames) {
        animationPreviewTimer = null;
        setAnimationPreviewStatus("Finished ".concat(totalFrames, " frame").concat(totalFrames === 1 ? "" : "s", " at ").concat(fps, " FPS."));
        return;
      }
      setAnimationPreviewFrame(animationPreviewData, frameSequence[framePointer], framePointer + 1, totalFrames);
      animationPreviewTimer = window.setTimeout(advanceFrame, 1000 / fps);
    };
    animationPreviewTimer = window.setTimeout(advanceFrame, 1000 / fps);
  };
  const renderAnimationPreviewModal = () => {
    const previewData = getAnimationPreviewData();
    if (!previewData) {
      return;
    }
    if (isAnimationPreviewOpen()) {
      closeAnimationPreviewModal();
    }
    stopAnimationPreviewPlayback({
      preserveStatus: true
    });
    normalizeAnimationPreviewState(previewData);
    animationPreviewData = previewData;
    const modal = addon.tab.createModal("Test animation", {
      isOpen: true,
      useEditorClasses: true
    });
    animationPreviewModal = modal;
    modal.backdrop.classList.add("sa-animation-backdrop");
    modal.container.classList.add("sa-animation-modal");
    modal.content.classList.add("sa-animation-modal-content");
    modal.closeButton.tabIndex = "0";
    modal.closeButton.setAttribute("role", "button");
    modal.backdrop.addEventListener("click", closeAnimationPreviewModal);
    modal.closeButton.addEventListener("click", closeAnimationPreviewModal);
    modal.closeButton.addEventListener("keydown", e => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        closeAnimationPreviewModal();
      }
    });
    animationPreviewKeydownHandler = e => {
      if (e.key === "Escape") {
        closeAnimationPreviewModal();
      }
    };
    window.addEventListener("keydown", animationPreviewKeydownHandler);
    animationPreviewResizeHandler = () => {
      if (isAnimationPreviewOpen()) {
        applyAnimationPreviewStageAspectRatio(animationPreviewStageAspectRatio);
      }
    };
    window.addEventListener("resize", animationPreviewResizeHandler);
    const shell = document.createElement("div");
    shell.className = "sa-animation-shell";
    modal.content.appendChild(shell);
    const intro = document.createElement("div");
    intro.className = "sa-animation-intro";
    intro.textContent = "Preview the costume range for ".concat(previewData.spriteName, ".");
    shell.appendChild(intro);
    const controls = document.createElement("div");
    controls.className = "sa-animation-controls";
    shell.appendChild(controls);
    const startSelect = document.createElement("select");
    startSelect.className = "sa-animation-input";
    const endSelect = document.createElement("select");
    endSelect.className = "sa-animation-input";
    const fpsInput = document.createElement("input");
    fpsInput.className = "sa-animation-input";
    fpsInput.type = "number";
    fpsInput.min = "1";
    fpsInput.max = "240";
    fpsInput.step = "1";
    const soundSelect = document.createElement("select");
    soundSelect.className = "sa-animation-input";
    for (const [index, costume] of previewData.costumes.entries()) {
      const option = document.createElement("option");
      option.value = String(index);
      option.textContent = "".concat(index + 1, ". ").concat(getAnimationPreviewCostumeName(costume, index));
      startSelect.appendChild(option.cloneNode(true));
      endSelect.appendChild(option);
    }
    const noAudioOption = document.createElement("option");
    noAudioOption.value = "-1";
    noAudioOption.textContent = "No audio";
    soundSelect.appendChild(noAudioOption);
    for (const [index, sound] of previewData.sounds.entries()) {
      const option = document.createElement("option");
      option.value = String(index);
      option.textContent = "".concat(index + 1, ". ").concat(sound.name || "Sound ".concat(index + 1));
      soundSelect.appendChild(option);
    }
    controls.appendChild(createAnimationPreviewField("Start costume", startSelect));
    controls.appendChild(createAnimationPreviewField("End costume", endSelect));
    controls.appendChild(createAnimationPreviewField("FPS", fpsInput));
    controls.appendChild(createAnimationPreviewField("Audio", soundSelect, {
      noteText: previewData.sounds.length ? "Uses sounds from the current sprite." : "No sounds found on the current sprite."
    }));
    const previewPanel = document.createElement("div");
    previewPanel.className = "sa-animation-preview";
    shell.appendChild(previewPanel);
    const stage = document.createElement("div");
    stage.className = "sa-animation-stage";
    previewPanel.appendChild(stage);
    const image = document.createElement("img");
    image.className = "sa-animation-image";
    image.draggable = false;
    stage.appendChild(image);
    const meta = document.createElement("div");
    meta.className = "sa-animation-meta";
    previewPanel.appendChild(meta);
    const frameName = document.createElement("div");
    frameName.className = "sa-animation-frame-name";
    meta.appendChild(frameName);
    const status = document.createElement("div");
    status.className = "sa-animation-status";
    meta.appendChild(status);
    const actions = document.createElement("div");
    actions.className = "sa-animation-actions";
    shell.appendChild(actions);
    const playButton = document.createElement("button");
    playButton.type = "button";
    playButton.className = "sa-animation-button";
    playButton.dataset.variant = "primary";
    playButton.textContent = "Play";
    actions.appendChild(playButton);
    const stopButton = document.createElement("button");
    stopButton.type = "button";
    stopButton.className = "sa-animation-button";
    stopButton.textContent = "Stop";
    actions.appendChild(stopButton);
    animationPreviewElements = {
      endSelect,
      fpsInput,
      frameName,
      image,
    ...
    startSelect.value = String(animationPreviewState.startCostumeIndex);
    endSelect.value = String(animationPreviewState.endCostumeIndex);
    fpsInput.value = String(animationPreviewState.fps);
    soundSelect.value = String(animationPreviewState.soundIndex);
    const syncAnimationPreviewState = () => {
      readAnimationPreviewStateFromModal();
      normalizeAnimationPreviewState(previewData);
      startSelect.value = String(animationPreviewState.startCostumeIndex);
      endSelect.value = String(animationPreviewState.endCostumeIndex);
      fpsInput.value = String(animationPreviewState.fps);
      soundSelect.value = String(animationPreviewState.soundIndex);
      updateAnimationPreviewSnapshot(previewData);
    };
    startSelect.addEventListener("change", syncAnimationPreviewState);
    endSelect.addEventListener("change", syncAnimationPreviewState);
    fpsInput.addEventListener("input", syncAnimationPreviewState);
    soundSelect.addEventListener("change", syncAnimationPreviewState);
  ...
  window.addEventListener(USER_LAYER_CHANGE_EVENT, () => {
    if (settings.enabled) {
      updateOnionLayers();
    }
  });
  ...
  animationPreviewButton = createButton();
  animationPreviewButton.addEventListener("click", renderAnimationPreviewModal);
  animationPreviewButton.title = "Test animation";
  animationPreviewButton.appendChild(createButtonImage("play"));
  ...
  toggleControlsGroup.append(settingPageWrapper, toggleButton, animationPreviewButton, settingButton);
