...
function createScalePoints(paper, lib, objects, sx, sy) {
  ...
  } : {}) : {}), _state_js__WEBPACK_IMPORTED_MODULE_0__["snapTo"].objectEdges ? Object.fromEntries(objects.filter(item => !(item.selected || item.data && item.data.isHelperItem || item.guide || item.layer && item.layer.data && (item.layer.data.hiddenByUser || item.layer.data.lockedByUser))).map(item => [sx && ["item_".concat(item.id, "_r"), {
    type: "xcoord",
  ...
  }]].filter(Boolean)).flat(1)) : {}), _state_js__WEBPACK_IMPORTED_MODULE_0__["snapTo"].objectMidlines ? Object.fromEntries(objects.filter(item => !(item.selected || item.data && item.data.isHelperItem || item.guide || item.layer && item.layer.data && (item.layer.data.hiddenByUser || item.layer.data.lockedByUser))).map(item => [sx && ["item_".concat(item.id, "_cx"), {
    type: "xcoord",
...
function createSnapPoints(paper, selectionBounds, lib, objects) {
  ...
  } : {}), _state_js__WEBPACK_IMPORTED_MODULE_0__["snapTo"].objectEdges ? Object.fromEntries(objects.filter(item => !(item.selected || item.data && item.data.isHelperItem || item.guide || item.layer && item.layer.data && (item.layer.data.hiddenByUser || item.layer.data.lockedByUser))).map(item => [["item_".concat(item.id, "_r"), {
    type: "itemSideVert",
    value: item.bounds.right,
    clamp: {
      min: item.bounds.top,
  ...
  }]]).flat(1)) : {}), _state_js__WEBPACK_IMPORTED_MODULE_0__["snapTo"].objectCenters ? Object.fromEntries(objects.filter(item => !(item.selected || item.data && item.data.isHelperItem || item.guide || item.layer && item.layer.data && (item.layer.data.hiddenByUser || item.layer.data.lockedByUser))).map(item => [["item_".concat(item.id, "_c"), {
    type: "point",
    value: item.bounds.center
  }]]).flat(1)) : {}), _state_js__WEBPACK_IMPORTED_MODULE_0__["snapTo"].objectMidlines ? Object.fromEntries(objects.filter(item => !(item.selected || item.data && item.data.isHelperItem || item.guide || item.layer && item.layer.data && (item.layer.data.hiddenByUser || item.layer.data.lockedByUser))).map(item => [["item_".concat(item.id, "_cx"), {
    type: "xcoord",
    value: item.bounds.center.x
  }], ["item_".concat(item.id, "_cy"), {
    type: "ycoord",
    value: item.bounds.center.y
  }]]).flat(1)) : {}), _state_js__WEBPACK_IMPORTED_MODULE_0__["snapTo"].objectCorners ? Object.fromEntries(objects.filter(item => !(item.selected || item.data && item.data.isHelperItem || item.guide || item.layer && item.layer.data && (item.layer.data.hiddenByUser || item.layer.data.lockedByUser))).map(item => [["item_".concat(item.id, "_tl"), {
    type: "point",
...
function loadModules(paper, vm) {
  ...
  const getSnappableObjects = function getSnappableObjects() {
    ...
    for (const layer of paper.project.layers) {
      ...
      const isUserLayer = !!(layer.data && layer.data.isUserLayer);
      const isLegacyPaintLayer = !!(layer.data && (layer.data.isPaintingLayer || layer.data.isRasterLayer));
      if (!isUserLayer && !isLegacyPaintLayer) {
        continue;
      }
      if (layer.data && (layer.data.hiddenByUser || layer.data.lockedByUser)) {
        continue;
      }
      for (const child of layer.children) {
        ...
        if (child.data && (child.data.isHelperItem || child.data.isLayerRaster)) {
          continue;
        }
        objects.push(child);
      }
...
const updateScaleTool = (paper, tool, vm) => {
  ...
  _compatibility_js__WEBPACK_IMPORTED_MODULE_2__["addons"].paintSnap = function (event) {
    ...
    const snappableObjects = getSnappableObjects();
    const snapXCoords = Object(_genScalePoints_js__WEBPACK_IMPORTED_MODULE_0__["default"])(paper, lib, snappableObjects, doesSx, false);
    const canSnapXCoords = Object.entries(snapXCoords).map(format).filter(canSnap).sort(sortByPrioOrDist);
    const snapYCoords = Object(_genScalePoints_js__WEBPACK_IMPORTED_MODULE_0__["default"])(paper, lib, snappableObjects, false, doesSy);
...
const updateSelectTool = (paper, tool, vm) => {
  ...
  function onMouseDrag(event) {
    ...
    if (!event.modifiers.shift && this.mode !== _helpers_js__WEBPACK_IMPORTED_MODULE_1__["Modes"].RESHAPE) {
      const snapPoints = Object(_genSnapPoints_js__WEBPACK_IMPORTED_MODULE_0__["default"])(paper, selectionBounds, lib, getSnappableObjects());
